...
if st.session_state.logged_in:
    ...
    if 'Server1' in config:
        ...
        remove_srv1 = st.Page(
            "Server1/remove_duplicates_srv1.py", title=f"{server1['name']} - Remover Duplicados"
        )
        anuncio_srv1 = st.Page(
            "Server1/anuncio_bgp_srv1.py", title=f"{server1['name']} - Anúncio BGP"
        )
    ...
    if 'Server2' in config:
        ...
        remove_srv2 = st.Page(
            "Server2/remove_duplicates_srv2.py", title=f"{server2['name']} - Remover Duplicados"
        )
        anuncio_srv2 = st.Page(
            "Server2/anuncio_bgp_srv2.py", title=f"{server2['name']} - Anúncio BGP"
        )
